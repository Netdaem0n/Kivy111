...
class SecondScr(Screen):
    def __init__(self, name='second'):
        ...
        btn = Button(text="Вернись, вернись!", font_size=24)
        btn.on_press = self.next
        self.add_widget(btn)
    ...
    def next(self):
        ...
        self.manager.current = 'RRR'


class RRRRScr(Screen):
    def __init__(self, name='RRR'):
        super().__init__(name=name)
        btn = Button(text="RRRRRRRRR, вернись!", font_size=24)
        btn.on_press = self.next
        self.add_widget(btn)
